...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train, opt.batch_size, shuffle = True, drop_last=True, num_workers=8)
    test_loader = DataLoader(test, opt.batch_size, shuffle=False, drop_last=False, num_workers=8)
    c0 = init_gbnn(train)
    ...
    for stage in range(opt.num_nets):
        ...
        train_loader = DataLoader(train, opt.batch_size, sampler = train_sampler, drop_last=True, num_workers=8)
        ################################################################################################
    ...
    fname = opt.data + '_clf_loss'
    np.savez(fname, all_ensm_losses, all_ensm_losses_te, all_mdl_losses)
