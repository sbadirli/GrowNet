...
def default_collate(batch):
    ...
    elif isinstance(batch[0], scipy.sparse.csr.csr_matrix):
        row_idx = []
        col_idx = []
        val = []
        for i, b in enumerate(batch):
            col = b.indices
            row_idx.extend([i] * len(col))
            col_idx.extend(col)
            val.extend(b.data)
        i = torch.LongTensor([row_idx, col_idx])
        v = torch.FloatTensor(val)
        return torch.sparse.FloatTensor(i, v, torch.Size([len(batch), batch[0].shape[1]]))
