...
def get_data():
    if opt.data in ['ca_housing', 'ailerons', 'YearPredictionMSD', 'slice_localization']:
        train = LibSVMRegData(opt.tr, opt.te, opt.feat_d, opt.normalization, opt.cv)
        ...
        if opt.cv:
            ...
            train.feat = train.feat[:cut]
            train.label = train.label[:cut]
            val.feat = val.feat[cut:]
            val.label = val.label[cut:]
    else:
        pass
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train, opt.batch_size, shuffle=False, drop_last=False, num_workers=2)
    test_loader = DataLoader(test, opt.batch_size, shuffle=False, drop_last=False, num_workers=2)
    if opt.cv:
        val_loader = DataLoader(val, opt.batch_size, shuffle=False, drop_last=False, num_workers=2)
        best_rmse = pow(10, 6)
    val_rmse = best_rmse
    ...
    tr_rmse, te_rmse = loss_models[vest_stage, 0], loss_models[bets_stage, 1]
    print(f'Best validarion stage: {stage}  RMSE@Tr: {tr_rmse:.5f}, final RMSE@Te: {te_rmse:.5f}')
    loss_models = loss_models.detach().cpu().numpy()
    fname =  opt.data +'_rmse'
