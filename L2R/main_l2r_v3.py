...
if __name__ == "__main__":
        ...
        for epoch in range(opt.epochs_per_stage):
            ...
            for q, y, x in train_loader.generate_query_batch(df_train, opt.batch_size):
                ...
                for i in range(len(uq)):
                    ...
                    #print(f'Best N value: {N}')
                    # Calculating gradient (1st or 2nd order)
                    #out_check = torch.sum(out_i, (0, 1)).item()
                    #if out_check == float('inf') or np.isnan(out_check):
                ...
                #print('Model parameters before grad update')
            ...
            for _ in range(opt.correct_epoch):
                ...
                for q, y, x in train_loader.generate_query_batch(df_train, opt.batch_size):
                    ...
                    for i in range(len(uq)):
                        ...
                        N = 1.0 / ideal_dcg.maxDCG(y_i) 
                        grad_ord1, grad_ord2 = grad_calc_(y_i, out_i, gain_type, opt.sigma, N, device)
                        out_i.backward(-grad_ord1/(grad_ord2*opt.batch_size), retain_graph=True)
    ...
    fname = opt.data + '_NDCG_pairwiseloss' + '_v3'
