...
def get_data():
    if opt.data == 'yahoo':
        data_fold = None
        train_loader, df_train, test_loader, df_test = load_train_test_data(data_fold, opt.data, opt.cv)
...
if __name__ == "__main__":
    ...
    for stage in range(opt.num_nets):
        ...
        for epoch in range(opt.epochs_per_stage):
			...
			grad_batch = gr_dir
                        ...
                        grad_batch = torch.cat((grad_batch, gr_dir), dim=0)
    ...
    np.savez('./results/' + opt.data + '_PW_parameters_' + opt.model_order, opt)
