...
if __name__ == "__main__":
        ...
        for epoch in range(opt.epochs_per_stage):
            for q, y, x in train_loader.generate_query_batch(df_train, opt.batch_size):
                ...
                if opt.cuda:
                    ...
                    y = torch.tensor(y/max_label, dtype=torch.float32, device=device).view(-1, 1)
                ...
                out = torch.as_tensor(out, dtype=torch.float32, device=device).view(-1, 1)
                #out = 1./(1. + torch.exp(-net_ensemble.boost_rate*out)) # Sigmoid
        ...
        if stage > 0:
            ...
            for _ in range(opt.correct_epoch):
                for q, y, x in train_loader.generate_query_batch(df_train, opt.batch_size):
                    ...
                    out = torch.as_tensor(out, dtype=torch.float32, device=device).view(-1, 1)
                    #out = 1./(1. + torch.exp(-out)) # Sigmoid
                    loss = loss_corr(out, y)
                    #net_ensemble.zero_grad()
                    optimizer.zero_grad()
                    loss.backward()
                    #scheduler.step()
                    optimizer.step()
                    stage_loss.append(loss.detach().cpu().numpy())
        sl = 0
        if stage_loss != []:
    ...
    fname = opt.data + '_NDCG_Logisticloss'
